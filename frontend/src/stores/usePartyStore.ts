...
export const usePartyStore = create<PartyStore>()(
  persist(
    (set, get) => ({
      ...
      addCharacterToParty: (character, slot) =>
        set(state => {
      ...
      updatePartyMemberAP: (characterIndex, newAp) =>
        set(state => {
          const updatedParty = [...state.party];
          const character = updatedParty[characterIndex];
          if (character) {
      ...
      updatePartyMemberStatusEffects: (characterIndex, effects) =>
        set(state => {
          const updatedParty = [...state.party];
          const character = updatedParty[characterIndex];
          if (character) {
      ...
      updatePartyMember: (characterIndex, updates) =>
        set(state => {
          const updatedParty = [...state.party];
          const character = updatedParty[characterIndex];
          if (character) {
      ...
      equipItem: (characterIndex, item, slot) =>
        set(state => {
      ...
      levelUpCharacter: characterIndex =>
        set(state => {
          ...
          // Check if this level grants a feat (every 4 levels)
          const grantsFeat = newLevel % 4 === 0;
      ...
      selectFeat: (characterIndex, feat, choice) =>
        set(state => {
          const updatedParty = [...state.party];
          const character = updatedParty[characterIndex];
          if (!character) return { party: updatedParty };
