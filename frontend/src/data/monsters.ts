        ...
        abilities: [
            {
                id: 'regeneration',
                name: 'Regeneration',
                description: 'Regains 10 HP at the start of its turn. Dies only if it starts turn with 0 HP and doesn\'t regenerate.',
                level: 1,
                type: 'passive'
            }
        ],
        ...
        abilities: [
            {
                id: 'fire_breath',
                name: 'Fire Breath',
                description: 'Exhales fire in a 30-foot cone. Each creature must make a DC 17 Dexterity save, taking 56 (16d6) fire damage on failed save, or half on success.',
                level: 1,
                type: 'active',
                cost: { AP: 0 }
            }
...
export function getRandomMonster(partyLevel: number): Enemy {
    ...
    const suitable = monsters.filter(m =>
        m.level >= partyLevel - levelRange &&
        m.level <= partyLevel + 1
    );
