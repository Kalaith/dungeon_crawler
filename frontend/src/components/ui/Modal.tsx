    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      >
        <div className="flex justify-between items-center p-5 border-b border-gray-400/20">
          <h3 className="text-xl font-semibold text-slate-900 dark:text-gray-200 m-0">{title}</h3>
          <button
            onClick={onClose}
        ...
        {footer && (
          <div className="flex justify-end gap-3 p-5 border-t border-gray-400/20">{footer}</div>
        )}
      </div>
    </div>
  );
