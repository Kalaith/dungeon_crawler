...
const generateRandomCharacter = (): Character => {
    ...
    const randomClass = simpleClasses[Math.floor(Math.random() * simpleClasses.length)] || characterClasses[0];
                    ...
                    {availableCharacters.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {availableCharacters.map((character) => (
                                <div
                                    key={character.id}
                                    className={`bg-etrian-700 rounded-lg p-6 border-2 transition-all cursor-pointer ${selectedCharacter?.id === character.id
