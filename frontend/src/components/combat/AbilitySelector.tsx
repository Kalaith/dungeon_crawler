...
export const AbilitySelector: React.FC<AbilitySelectorProps> = ({
    ...
    const availableAbilities = abilities.filter(ability => ability.level <= currentLevel);

    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[80vh] overflow-hidden">
                <div className="p-4 border-b border-gray-200 dark:border-gray-700">
                    <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Use Ability</h2>
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex justify-between items-start mb-1">
                                            <span className={`font-bold ${canUse ? 'text-gray-900 dark:text-white' : 'text-gray-500'}`}>
                                                {ability.name}
                                            </span>
                                            {apCost > 0 && (
