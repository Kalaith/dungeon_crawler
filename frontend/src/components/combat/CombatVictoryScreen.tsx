    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/95 animate-in fade-in duration-300">
            <div className="bg-stone-800 border-4 border-yellow-600 p-8 rounded-lg max-w-2xl w-full text-center shadow-2xl">
                <h2 className="text-5xl font-bold text-yellow-500 mb-8 tracking-wider drop-shadow-md">VICTORY!</h2>

                <div className="space-y-6 mb-10">
                    {/* Basic Rewards */}
                    <div className="bg-stone-900/50 p-6 rounded-lg border border-stone-700">
                        <h3 className="text-xl text-stone-400 mb-4 uppercase tracking-widest font-bold">Rewards</h3>
                    ...
                    {victoryData.items.length > 0 && (
                        <div className="bg-stone-900/50 p-6 rounded-lg border border-stone-700">
                            <h3 className="text-xl text-stone-400 mb-4 uppercase tracking-widest font-bold">Loot Found</h3>
                            <div className="flex flex-wrap justify-center gap-3">
                                {victoryData.items.map((item, i) => (
                                    <div key={i} className={`px-4 py-2 rounded bg-stone-800 border-2 flex items-center gap-2 ${item.rarity === 'legendary' ? 'text-orange-400 border-orange-500 shadow-orange-500/20 shadow-lg' :
                                            item.rarity === 'epic' ? 'text-purple-400 border-purple-500 shadow-purple-500/20 shadow-lg' :
                                                item.rarity === 'rare' ? 'text-blue-400 border-blue-500' :
                            ...
                            <div className="space-y-2">
                                {victoryData.levelUps.map((levelup, i) => (
                                    <div key={i} className="text-xl text-stone-200">
                                        <span className="font-bold text-white">{levelup.name}</span> gained {levelup.levelsGained} level(s)!
                                    </div>
                                ))}
                            </div>
                        </div>
