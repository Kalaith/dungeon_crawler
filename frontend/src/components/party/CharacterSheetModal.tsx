            ...
            <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                    {Object.entries(character.attributes).map(([attr, val]) => (
                        <div key={attr} className="flex justify-between items-center border-b border-gray-700 pb-1">
                ...
                <div className="space-y-3">
                    <div>
                        <div className="flex justify-between text-sm mb-1">
                            ...
                            <span className="text-white">{character.derivedStats.HP.current} / {character.derivedStats.HP.max}</span>
                        </div>
                        <StatBar current={character.derivedStats.HP.current} max={character.derivedStats.HP.max} color="health" />
                    </div>
                    <div>
                        <div className="flex justify-between text-sm mb-1">
                            <span className="text-gray-400">Astral Points</span>
                            <span className="text-white">{character.derivedStats.AP.current} / {character.derivedStats.AP.max}</span>
                        </div>
                        <StatBar current={character.derivedStats.AP.current} max={character.derivedStats.AP.max} color="mana" />
                    </div>
                    <div className="grid grid-cols-2 gap-4 mt-4">
                        <div className="bg-gray-900 p-2 rounded text-center">
                            <div className="text-xs text-gray-500 uppercase">Initiative</div>
                            <div className="text-xl font-bold text-yellow-400">{character.derivedStats.Initiative}</div>
                        </div>
                        <div className="bg-gray-900 p-2 rounded text-center">
                        ...
                        <div className="bg-gray-900 p-2 rounded text-center">
                            ...
                            <div className="text-xl font-bold text-green-400">+{character.derivedStats.Proficiency}</div>
                        </div>
                        <div className="bg-gray-900 p-2 rounded text-center">
                            <div className="text-xs text-gray-500 uppercase">Movement</div>
                            <div className="text-xl font-bold text-white">{character.derivedStats.Movement} ft</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-gray-700">
                    {character.inventory.length === 0 ? (
                        <tr>
                            <td colSpan={3} className="px-4 py-8 text-center text-gray-500 italic">Inventory is empty</td>
                        </tr>
                    ) : (
                        character.inventory.map((item, idx) => (
                            <tr key={`${item.id}-${idx}`} className="hover:bg-gray-700/50">
                                <td className="px-4 py-3 font-medium text-white">
                                    ...
                                    {item.quantity && item.quantity > 1 && <span className="text-gray-400 ml-2">x{item.quantity}</span>}
                                </td>
            ...
            <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                    {character.skills.map((skill) => (
                        <div key={skill.id} className="bg-gray-800 p-2 rounded border border-gray-700 flex justify-between items-center">
            ...
            <div>
                ...
                <div className="space-y-2">
                    {character.feats.map((feat, idx) => (
                        <div key={`${feat.id}-${idx}`} className="bg-gray-800 p-3 rounded border border-gray-700">
                            <div className="font-bold text-white mb-1">{feat.name}</div>
                            <p className="text-sm text-gray-400">{feat.description}</p>
                        </div>
                    ))}
                    {character.feats.length === 0 && <p className="text-gray-500 italic">No feats acquired.</p>}
                </div>
            </div>
                ...
                <div>
                    ...
                    <div className="space-y-2">
                        {character.spells.map((spell, idx) => (
                            <div key={`${spell.name}-${idx}`} className="bg-gray-800 p-3 rounded border border-gray-700">
                        ...
                        {character.spells.length === 0 && <p className="text-gray-500 italic">No spells known.</p>}
                    </div>
                </div>
            )}
        </div>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <div className="text-cyan-400 flex items-center gap-2">
                                <span>Level {character.level} {character.race.name} {character.class.name}</span>
